...
// Array of general images used in the carousel
const images = [
  image1, image2, image3, image4, image5, image6, 
  image7, image9, image10, image11
];

// Array of noodle-related images for display
...
function Dashboard() {
  ...
  const [currentNoodleImage, setCurrentNoodleImage] = useState(0); // State to track the current noodle image index

  // Function to handle mouse enter event for pausing the slider
  const handleMouseEnter = () => {
    sliderContainerRef.current.classList.add('paused');
  };

  // Function to handle mouse leave event for resuming the slider
  const handleMouseLeave = () => {
    sliderContainerRef.current.classList.remove('paused');
  };

  // useEffect to set up the interval for changing noodle images every 5 seconds
  useEffect(() => {
...
export default Dashboard;// Exporting the Dashboard component to be used in app.js
